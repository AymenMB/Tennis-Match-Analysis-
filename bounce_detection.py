...
class BounceDetection:
    ...
    def postprocess(self, ind_bounce, preds):
        """
        Filters bounce indices to keep only the most likely frame within consecutive sequences.
        """
        if not ind_bounce.any():
            return []
        ...
        groups = []
        current_group = [ind_bounce[0]]
        for i in range(1, len(ind_bounce)):
            if ind_bounce[i] == ind_bounce[i-1] + 1:
                current_group.append(ind_bounce[i])
            else:
                groups.append(current_group)
                current_group = [ind_bounce[i]]
        groups.append(current_group) # Add the last group

        # Find the index with the max prediction score in each group
        for group in groups:
            if len(group) == 1:
                ind_bounce_filtered.append(group[0])
            else:
                # Get prediction scores for the indices in the current group
                group_preds = preds[group]
                # Find the index within the group that has the maximum prediction score
                max_pred_index_in_group = np.argmax(group_preds)
                # Get the original index corresponding to this max prediction
                best_index = group[max_pred_index_in_group]
                ind_bounce_filtered.append(best_index)
